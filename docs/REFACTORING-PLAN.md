...
**Головний висновок.** Проект має відмінну *документацію намірів* (CLAUDE.md +
skill), але слабкий *механічний примус*. Правила описані словами, а не
...
const marginClass =
  size === 'xs'
    ? position === 'start' ? 'mie-0.25' : 'mis-0.25'
    : position === 'start' ? 'mie-0.5' : 'mis-0.5';

// має бути — lookup-мапа + значення зі шкали
const ICON_MARGIN = {
  'xs:start': 'mie-0.5', 'xs:end': 'mis-0.5',
  'md:start': 'mie-0.5', 'md:end': 'mis-0.5',
} as const;
```
...
собі.** Довжина файлу — це *проксі-метрика*. Вона корелює з «забагато
...
Схема загалом хороша: коментарі пояснюють *чому* (рідкість), індекси продумані,
...
```ts
orderBy: { status: dir }  // ← не працює як треба
// →
...
- `src/icons/` має 5 підпапок (`achievements`, `base`, `brand`, `colorful`,
  `status`) — `colorful` описує *вигляд*, решта — *домен*. Перейменувати за
  доменом.
...
- **CLAUDE.md** — *процес*: definition of done, команди, тестова політика,
  документаційна політика, CI-гейти. Один розділ «Conventions» із посиланням
  на skill.
- **skill** — *код*: усе про те, як виглядає правильний файл (стилі, токени,
...
*Обсяг.* CLAUDE.md (~3.5k токенів) плюс скіл (~8.5k) вантажилися в кожну
сесію — ~12k токенів ще до того, як прочитано хоч рядок коду.

*Дрейф.* Обидва файли описували структуру проекту руками. Рукописний опис
  ...
  forbidden: [
    {
      ...
      to: {
        path:
          '^apps/web/src/(components/(repertoire|profile|metronome)|lib/hooks|lib/api|hooks/global|data)',
      },
    },
    {
...
контракт компонента — що *треба* передати. Опціональні — це вже налаштування.
...
| 12 | `feat(api): BandMembershipGuard + exception filter` | 3.3 | середній | високий |
